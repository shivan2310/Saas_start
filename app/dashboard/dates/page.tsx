  ...
  return (
    <div className="space-y-8 max-w-5xl">
      <div className="flex items-end justify-between">
        <h2 className="text-[24px] font-semibold tracking-tight text-dash-text">Important Dates</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="rounded-lg border border-dash-border bg-dash-card p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1">
                <button onClick={prevMonth} className="p-1 hover:bg-dash-hover rounded text-dash-text-muted hover:text-dash-text transition-colors">
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <button onClick={nextMonth} className="p-1 hover:bg-dash-hover rounded text-dash-text-muted hover:text-dash-text transition-colors">
              ...
              {days.map(day => {
                ...
                return (
                  <div 
                    key={day} 
                    className={cn(
                      "h-8 flex items-center justify-center text-[12px] rounded-full transition-colors cursor-default",
            ...
            <form onSubmit={submit} className="bg-dash-card border border-dash-border rounded-lg p-4 space-y-4">
              <div className="space-y-3">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-b border-dash-border text-[14px] text-dash-text-secondary focus:border-dash-text focus:outline-none py-1"
                  required
                />
                <input
                  type="text"
        ...
        <div className="md:col-span-7">
          <div className="space-y-4">
            ...
            {loading ? (
              <div className="text-[13px] text-dash-text-muted">Loading dates...</div>
            ) : items.length === 0 ? (
              <div className="text-[13px] text-dash-text-muted">No important dates recorded.</div>
            ) : (
              <div className="space-y-2">
                {items.map((item) => (
                  <div key={item.id} className="group flex items-start justify-between py-3 px-3 -mx-3 hover:bg-dash-hover rounded-lg transition-colors border-l-2 border-transparent hover:border-dash-accent">
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                      <span className="flex items-center gap-1.5 text-[13px] font-medium text-dash-text-secondary bg-dash-card border border-dash-border px-2.5 py-1 rounded">
                        <CalendarIcon className="h-3 w-3" />
                        {item.date}
                      </span>
                      <button 
                        onClick={() => remove(item.id)}
                        className="text-dash-text-muted hover:text-dash-text opacity-0 group-hover:opacity-100 transition-all p-1"
                      >
