...
export default function ExpensesPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Add an expense</CardTitle></CardHeader><CardContent><form onSubmit={submit} className="grid gap-3 sm:grid-cols-4"><Input label="What was it?" value={description} maxLength={200} onChange={(e) => setDescription(e.target.value)} placeholder="Coffee, groceries..." required /><Input label="Amount" type="number" min="0" max="100000000" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" required /><div className="w-full flex flex-col gap-1.5"><label htmlFor="expense-category" className="text-xs font-medium text-black uppercase tracking-wider">Category</label><select id="expense-category" value={category} onChange={(e) => setCategory(e.target.value)} className="flex h-10 w-full rounded border border-border bg-white px-3 py-2 text-sm text-black focus:outline-none focus:ring-1 focus:ring-black"><option>Food</option><option>Transport</option><option>Shopping</option><option>Bills</option><option>Health</option><option>Entertainment</option><option>Education</option><option>Travel</option><option>Home</option><option>Salary</option><option>Custom</option></select></div>{category === "Custom" && <Input label="Custom category" maxLength={100} value={customCategory} onChange={(e) => setCustomCategory(e.target.value)} placeholder="e.g. Pets" required />}<Button type="submit" className="self-end">Add expense</Button></form></CardContent></Card>
